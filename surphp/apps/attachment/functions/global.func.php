<?php
	...
	function file_list($dirpath,$currentdir) {
		...
		$list['local'] = str_replace(array(SUR_PATH, DIRECTORY_SEPARATOR.DIRECTORY_SEPARATOR), array('',DIRECTORY_SEPARATOR), $filepath);
		return $list;
	}
	...
	function initupload($module,$args, $userid, $isadmin = '0'){
		...
		$sess_id = SYS_TIME;
		$swf_auth_key = md5(surphp::load_config('config','AUTH_KEY').$sess_id);
		$init =  'var swfu = \'\';
		$(document).ready(function(){
		swfu = new SWFUpload({
			flash_url:"'.JS_PATH.'swfupload/swfupload.swf?"+Math.random(),
			upload_url:"'.SITE_URL.'index.php?m=attachment&c=index&a=swfupload&dosubmit=1",
