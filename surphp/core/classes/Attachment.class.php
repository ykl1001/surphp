...
class Attachment {
	...
	function __construct($module='',$upload_dir = '') {
		...
		surphp::load_core_func('dir');		
		surphp::load_core_class('image');
		$this->upload_root = surphp::load_config('config','UPLOAD_PATH');
		$this->upload_func = 'copy';
		$this->upload_dir = $upload_dir;//可指定路径，默认空
	...
	function upload($field, $alowexts = '', $maxsize = 0, $overwrite = 0,$thumb_setting = array(), $watermark_enable = 1) {
		...
		if(empty($alowexts) || $alowexts == '') {
			$site_setting = $this->_get_site_setting();
	...
	{
		$upload_url = surphp::load_config('config','UPLOAD_URL') ? surphp::load_config('config','UPLOAD_URL') : SITE_URL.$this->upload_root;
		$this->field = $field;
		$dir = date('Y/md/');
	...
	*/
	...
	function uploaderror($id = 0)	{
		file_put_contents(SUR_PATH.'xxx.txt', $id);
	}
	 ...
	 * @param  $siteid 站点id
	 */
	private function _get_site_setting($siteid=1) {
