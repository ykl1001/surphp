<?php
surphp::load_core_class('Db_Factory', '', 0);
...
class Model {
	...
	public function __construct($table_name = '') {
	    $this->db          = Db_Factory::get_instance()->generate_db();
	    $this->db_tablepre = surphp::load_config('config','DB_PREFIX');
	 ...
	 * @return unknown_type
	 */
	final public function listinfo($where = '', $order = '', $page = 1, $pagesize = 20, $key='', $data = '*') {
		$where = $this->sqls($where);
		$this->number = $this->count($where);
		$page = max(intval($page), 1);
		$offset = $pagesize*($page-1);
		if ($this->number > 0) {
		    surphp::load_core_class('page');
		    $page = new Page($this->number, $page,$pagesize);
