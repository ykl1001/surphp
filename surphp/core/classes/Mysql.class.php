...
class Mysql {
	...
	public function connect() {
		$func = 'mysql_connect';
		if(!$this->link = @$func($this->config['DB_HOST'].':'.$this->config['DB_PORT'], $this->config['DB_USER'], $this->config['DB_PWD'], 1)) {
			$this->halt('Can not connect to MySQL server');
			return false;
		}

		if($this->version() > '4.1') {
			$charset = isset($this->config['DB_CHARSET']) ? $this->config['DB_CHARSET'] : '';
	...
	public function halt($message = '', $sql = '') {
		if($this->config['DEVELOP_MODE']) {
			$this->errormsg = "<b>MySQL Query : </b> $sql <br /><b> MySQL Error : </b>".$this->error()." <br /> <b>MySQL Errno : </b>".$this->errno()." <br /><b> Message : </b> $message <br /><a href='http://www.baidu.com/s?wd=".urlencode($this->error())."' target='_blank' style='color:red'>Need Help?</a>";
			$msg = $this->errormsg;
