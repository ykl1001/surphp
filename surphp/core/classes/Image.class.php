...
class Image {
    ...
    function __construct($thumb_enable = 0 ,$siteid=1) {
    	$this->thumb_enable = $thumb_enable;
    	if($siteid) {
	    	$this->siteinfo = siteinfo($siteid);
	    	$this->site_setting = string2array($this->siteinfo['setting']);
	...
	function watermark($source, $target = '', $w_pos = '', $w_img = '', $w_text = 'surphp',$w_font = 8, $w_color = '#ff0000') {
		...
		if(!empty($w_img) && file_exists($w_img)) {
			...
			$ifwaterimage = 0;
			$temp = imagettfbbox(ceil($w_font*2.5), 0, CORE_PATH.'data'.DIRECTORY_SEPARATOR.'font'.DIRECTORY_SEPARATOR.'elephant.ttf', $w_text);
			$width = $temp[2] - $temp[6];
